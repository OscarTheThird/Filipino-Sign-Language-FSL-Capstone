...
// Filipino Sign Language WH Questions
const whquestionsData = [
    { question: 'What', video: '/PICTURES/fsl_basic_wh_questions/ANO.mp4' },
    { question: 'Why', video: '/PICTURES/fsl_basic_wh_questions/BAKIT.mp4' },
    { question: 'When', video: '/PICTURES/fsl_basic_wh_questions/KAILAN.mp4' },
    { question: 'How', video: '/PICTURES/fsl_basic_wh_questions/PAANO.mp4' },
    { question: 'Where', video: '/PICTURES/fsl_basic_wh_questions/SAAN.mp4' },
    { question: 'Who', video: '/PICTURES/fsl_basic_wh_questions/SINO.mp4' },
];
...
const QUIZ_ID = 'wh-questions-quiz';
...
function handleQuizAbandonedInOtherTab() {
    ...
    showAutoCloseMessage('Quiz was abandoned in another tab. Closing this tab...', () => {
        window.location.href = 'HTML/Lesson/whquestions.html';
    });
}
...
function generateQuestions() {
    ...
    const maxQuestions = Math.min(10, whquestionsData.length);
    const shuffled = shuffleArray([...whquestionsData]);
    questions = shuffled.slice(0, maxQuestions).map(item => ({
        correctAnswer: item.question,
        video: item.video,
        options: generateOptions(item.question)
    }));
}
...
function generateOptions(correctLetter) {
    ...
    const availableLetters = whquestionsData.map(item => item.question).filter(question => question !== correctLetter);
...
async function loadPreviousQuizData() {
    ...
    try {
        const progressRef = doc(db, 'users', currentUser.uid, 'progress', 'wh-questions-quiz');
...
async function saveFinalQuizResultsToFirestore(finalScore, total, percentage) {
    ...
    // Reference to the user's progress document for wh-questions-quiz
    const progressRef = doc(db, 'users', currentUser.uid, 'progress', 'wh-questions-quiz');
...
async function handleVisibilityChange() {
    ...
    const isOnQuizPage = currentPath.includes('whquestionsquiz.html');
    ...
    if (user) {
        ...
        if (navigator.onLine) {
            ...
            if (activeSession && activeSession.active && !isOnQuizPage) {
                ...
                window.location.href = basePath + '/HTML/Quiz/whquestionsquiz.html';
            }
        }
    } else {
        currentUser = null;
