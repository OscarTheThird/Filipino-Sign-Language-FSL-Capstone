...
// Filipino Sign Language Numbers 1-10
const numbersData = [
    { number: '1', video: '/PICTURES/fsl_numbers/1.mp4' },
    { number: '2', video: '/PICTURES/fsl_numbers/2.mp4' },
    { number: '3', video: '/PICTURES/fsl_numbers/3.mp4' },
    { number: '4', video: '/PICTURES/fsl_numbers/4.mp4' },
    { number: '5', video: '/PICTURES/fsl_numbers/5.mp4' },
    { number: '6', video: '/PICTURES/fsl_numbers/6.mp4' },
    { number: '7', video: '/PICTURES/fsl_numbers/7.mp4' },
    { number: '8', video: '/PICTURES/fsl_numbers/8.mp4' },
    { number: '9', video: '/PICTURES/fsl_numbers/9.mp4' },
    { number: '10', video: '/PICTURES/fsl_numbers/10.mp4' },
];
...
function handleQuizAbandonedInOtherTab() {
    ...
    showAutoCloseMessage('Quiz was abandoned in another tab. Closing this tab...', () => {
        window.location.href = 'HTML/Lesson/numbers.html';
    });
}
...
function generateQuestions() {
    ...
    const maxQuestions = Math.min(10, numbersData.length);
    const shuffled = shuffleArray([...numbersData]);
    questions = shuffled.slice(0, maxQuestions).map(item => ({
        correctAnswer: item.number,
        video: item.video,
        options: generateOptions(item.number)
    }));
}
...
function generateOptions(correctLetter) {
    ...
    const availableLetters = numbersData.map(item => item.number).filter(number => number !== correctLetter);
...
async function loadPreviousQuizData() {
    ...
    try {
        const progressRef = doc(db, 'users', currentUser.uid, 'progress', 'numbers-quiz');
...
async function saveFinalQuizResultsToFirestore(finalScore, total, percentage) {
    ...
    // Reference to the user's progress document for numbers-quiz
    const progressRef = doc(db, 'users', currentUser.uid, 'progress', 'numbers-quiz');
...
async function handleVisibilityChange() {
    ...
    const isOnQuizPage = currentPath.includes('numbersquiz.html');
    ...
    if (user) {
        ...
        if (navigator.onLine) {
            ...
            if (activeSession && activeSession.active && !isOnQuizPage) {
                ...
                window.location.href = basePath + '/HTML/Quiz/numbersquiz.html';
            }
        }
    } else {
        currentUser = null;
