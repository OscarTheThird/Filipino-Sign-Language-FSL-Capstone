...
function generateOptions(correctLetter) {
    ...
    const availableLetters = phrasesData.map(item => item.phrase).filter(letter => letter !== correctLetter);
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        if (navigator.onLine) {
            ...
            const isOnQuizPage = currentPath.includes('phrasesquiz.html');
