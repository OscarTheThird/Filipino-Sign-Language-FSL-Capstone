...
const familyMembersData = [
    { member: 'TATAY', video: '/PICTURES/fsl_family_members/TATAY.mp4' },
    { member: 'NANAY', video: '/PICTURES/fsl_family_members/NANAY.mp4' },
    { member: 'KUYA', video: '/PICTURES/fsl_family_members/KUYA.mp4' },
    { member: 'ATE', video: '/PICTURES/fsl_family_members/ATE.mp4' }
];
...
// Generate quiz questions (all 4 members)
function generateQuestions() {
    const shuffled = shuffleArray([...familyMembersData]);
...
function generateOptions(correctMember) {
    const options = [correctMember];
    const availableMembers = familyMembersData.map(item => item.member).filter(member => member !== correctMember);
...
async function saveQuizProgress() {
    ...
    try {
        const quizRef = doc(db, 'users', currentUser.uid, 'quizzes', 'familymembers');
