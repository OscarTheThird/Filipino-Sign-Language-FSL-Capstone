...
function generateOptions(correctAnswer) {
    ...
    const availableOptions = familymembersData.map(item => item.member).filter(opt => opt !== correctAnswer);
...
async function saveActiveQuizSession() {
    ...
    try {
        const sessionRef = doc(db, 'users', currentUser.uid, 'activeQuiz', 'family-members-quiz');
