...
// Filipino Sign Language Emergency & Basic Needs
const emergencyData = [
    { need: 'Help Me', video: '/PICTURES/fsl_emergency/TULUNGAN.mp4' },
    { need: 'Water', video: '/PICTURES/fsl_emergency/TUBIG.mp4' },
    { need: 'Eat', video: '/PICTURES/fsl_emergency/KAIN.mp4' },
    { need: 'Food', video: '/PICTURES/fsl_emergency/PAGKAIN.mp4' },
    { need: 'Stop', video: '/PICTURES/fsl_emergency/HINTO.mp4' },
    { need: 'Drink', video: '/PICTURES/fsl_emergency/INOM.mp4' },
];
...
function generateQuestions() {
    ...
    const maxQuestions = Math.min(10, emergencyData.length);
    const shuffled = shuffleArray([...emergencyData]);
    questions = shuffled.slice(0, maxQuestions).map(item => ({
        correctAnswer: item.need,
        video: item.video,
        options: generateOptions(item.need)
    }));
}
...
function generateOptions(correctLetter) {
    ...
    const availableLetters = emergencyData.map(item => item.need).filter(letter => letter !== correctLetter);
...
async function loadPreviousQuizData() {
    ...
    try {
        const progressRef = doc(db, 'users', currentUser.uid, 'progress', 'emergency-quiz');
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        if (navigator.onLine) {
            ...
            const isOnQuizPage = currentPath.includes('emergencyquiz.html');
            ...
            if (activeSession && activeSession.active && !isOnQuizPage) {
                ...
                window.location.href = basePath + '/HTML/Quiz/emergencyquiz.html';
            }
        }
    } else {
        currentUser = null;
