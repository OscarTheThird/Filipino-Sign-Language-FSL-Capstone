...
function generateOptions(correctAnswer) {
    ...
    const availableOptions = educationalData.map(item => item.term).filter(opt => opt !== correctAnswer);
...
async function saveActiveQuizSession() {
    ...
    try {
        const sessionRef = doc(db, 'users', currentUser.uid, 'activeQuiz', 'educational-context-quiz');
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        if (navigator.onLine) {
            ...
            const isOnQuizPage = currentPath.includes('educationalcontextquiz.html');
            ...
            if (activeSession && activeSession.active && !isOnQuizPage) {
                ...
                window.location.href = basePath + '/HTML/Quiz/educationalcontextquiz.html';
            }
        }
    } else {
        currentUser = null;
