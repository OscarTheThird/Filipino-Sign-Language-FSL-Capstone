...
// Generate quiz questions (all 10 numbers)
...
function generateOptions(correctNumber) {
    ...
    const availableNumbers = numbersData.map(item => item.number).filter(number => number !== correctNumber);
...
async function saveQuizProgress() {
    ...
    try {
        const quizRef = doc(db, 'users', currentUser.uid, 'quizzes', 'numbers');
