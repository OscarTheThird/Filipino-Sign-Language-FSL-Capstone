...
// Generate quiz questions (all 3 markers)
...
function generateOptions(correctMarker) {
    const options = [correctMarker];
    const availableMarkers = timeMarkersData.map(item => item.marker).filter(marker => marker !== correctMarker);
    const shuffled = shuffleArray(availableMarkers);
...
async function saveQuizProgress() {
    ...
    try {
        const quizRef = doc(db, 'users', currentUser.uid, 'quizzes', 'timemarkers');
